...
**ratewatch** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `ratewatch` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
ratewatch ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
ratewatch ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
ratewatch ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `ratewatch` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="ratewatch") for rec in records]   # records = your JSON output
